...
def get_masks_hierarchy(masks, masks_meta):
    order = sorted(list(enumerate(masks_meta)), key=lambda x: x[1][1])[::-1]            # 根據面積排序
    hierarchy = defaultdict(list)

    def check_inter(i, j):      # 检查交集，
    ...
    used_masks = np.zeros(len(masks), dtype=np.bool8)
    parents = [None] * len(masks)
    node_level = [0] * len(masks)
    # 计算每个mask与其他mask的关系，
    for ti in range(len(masks) - 1):
        for tj in range(ti + 1, len(masks)):
            i = order[ti][0]                # 第 i 个mask
            j = order[tj][0]                # 第 j 个mask

            assert i != j
            if used_masks[j] or not check_inter(i, j):      # 判断两个mask是否被使用，或者其IOU是否大于0.7， 这里如果没有交集，就不会后续的处理
                continue
    ...
    for global_id, obj_mask in ordered_masks:
        for layer_indx, (layer_mask, layer_objs) in enumerate(zip(layers, layers_objs)):
            if len(layer_objs) >= 255:
                continue
            if np.all(layer_mask[obj_mask] == 0):   # 全为空的部分
