...
from scripts.annotations_conversion import ade20k, coco_lvis, love_da


def parse_args():
    parser = argparse.ArgumentParser()

    parser.add_argument('dataset', choices=['ade20k', 'coco_lvis', 'loveda',], help='')
    parser.add_argument('--split', nargs='+', choices=['train', 'val', 'test', 'Train', 'Val'], type=str, default=['train', 'val'],
                        help='')
...
def main():
    ...
    for split in args.split:
        if args.dataset == 'openimages':
            ...
            love_da.create_annotations(Path(cfg.LOVEDA_PATH), dataset_split=split)
