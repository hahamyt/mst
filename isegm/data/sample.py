...
class DSample:
    ...
    def augment(self, augmentator):
        ...
        if self._encoded_masks.shape[:2] != self.image.shape[:2]:
            resized_mask = cv2.resize(self._encoded_masks, (self.image.shape[1], self.image.shape[0]), interpolation=cv2.INTER_NEAREST)
            self._encoded_masks = resized_mask.reshape(self.image.shape[1], self.image.shape[0], self._encoded_masks.shape[-1])

        aug_output = augmentator(image=self.image, mask=self._encoded_masks, is_check_shapes=False)
