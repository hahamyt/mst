...
class SBDDataset(ISDataset):
    ...
    def remove_buggy_masks(self, index, instances_mask):
        ...
        for obj_id in instances_ids:
            ...
            # 标记连通区域
            labeled_mask, num_features = label(obj_mask)
            ...
            # 计算每个连通区域的大小
            region_sizes = [(labeled_mask == region).sum() for region in range(1, num_features + 1)]

            # 找到最大区域的标签
            largest_region = np.argmax(region_sizes) + 1

            # 保留最大区域，其余部分置为 0
            obj_mask[labeled_mask != largest_region] = 0
            instances_mask[obj_mask == 0] = 0
