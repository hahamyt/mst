...
class PatchLoss(nn.Module):
    ...
    def forward(self, f_q, f_k, tau=0.07):
        ...
        for block in range(len(f_q)):
            ...
            f_k_resized_l = F.interpolate(f_k, scale_factor=1 / 32, mode='nearest')  # [B, C, H_l, W_l]
...
class ScoreLoss(nn.Module):
    ...
    def focal_loss(self, pred, target):
        ...
        neg_loss = -(1 - self.alpha) * torch.pow(pred, self.gamma) * torch.log(1 - pred) * neg_mask
    ...
    def forward(self, f_q, gt):
        ...
        for block in range(len(f_q)):
            ...
            focal_loss_s = self.focal_loss(f_q[block]['pos_scores_s'][mask_s], gt8[mask_s])
            focal_loss_l = self.focal_loss(f_q[block]['pos_scores_l'][mask_l], gt28[mask_l])

            loss = loss + 0.5 * (focal_loss_s + focal_loss_l)
