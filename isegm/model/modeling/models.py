...
class ScoreBlock(nn.Module):
    def __init__(self, in_dim, sr_ratio=1):
        ...
        self.cross_attn = MoECrossAttention(in_dim, num_heads=4, sr_ratio=sr_ratio)
        self.cross_mlp = Mlp(in_features=in_dim,hidden_features=in_dim,  drop=0.01)
...
class MoECrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None,
        ...
        self.proj = MoEMlp(
            in_features=dim,
            hidden_features=mlp_hidden_dim//4,
            out_features=dim,
            act_layer=nn.GELU,
            drop=proj_drop
        )
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_drop=0.0, qkv_bias=False, attn_drop=0.,
        ...
        if self.use_score:
            ...
            self.score_cross_attn = CrossAttention(dim, num_heads=4)

            self.norm6 = norm_layer(dim)
            self.norm7 = norm_layer(dim)
            self.score_l_s = CrossAttention(dim, num_heads=2)
            self.gamma = nn.Parameter(torch.ones(1, 1, dim), requires_grad=True)
            self.gamma2 = nn.Parameter(torch.ones(1, 1, dim), requires_grad=True)
