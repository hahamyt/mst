...
def interpolate_pos_embed(model, checkpoint_model):
    if 'pos_embed' in checkpoint_model:
    ...
    elif 'pos_embed' in list(checkpoint_model.keys())[0]:
        pos_embed_checkpoint = checkpoint_model[list(checkpoint_model.keys())[0]]
        embedding_size = pos_embed_checkpoint.shape[-1]
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            new_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
            checkpoint_model[list(checkpoint_model.keys())[0]] = new_pos_embed
    elif 'pos_embed' in list(checkpoint_model.keys())[3]:
        pos_embed_checkpoint = checkpoint_model[list(checkpoint_model.keys())[3]]
        embedding_size = pos_embed_checkpoint.shape[-1]
        num_patches = model.backbone.patch_embed.num_patches
        num_extra_tokens = 1  # model.pos_embed.shape[-2] - num_patches      # 去点cls的token
        # height (== width) for the checkpoint position embedding
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            new_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
            checkpoint_model[list(checkpoint_model.keys())[3]] = new_pos_embed
...
def interpolate_pos_embed_inference(model, infer_img_size, device):
    ...
    if orig_size != new_size:
        ...
        model.patch_embed.img_size = infer_img_size
