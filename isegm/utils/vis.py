...
def vis_result_base(image, pred_mask, instances_mask, iou, num_clicks, clicks_list, prev_prediction, last_y,
    ...
    fusion_prev = vis_mask_on_image(image, prev_prediction, vis_trimap=True, mask_color=mask_color,
    ...
    fusion_gt = add_tag(fusion_gt, 'ground truth')
    fusion_gt = cv2.copyMakeBorder(fusion_gt, 0, 0, 10, 10, cv2.BORDER_CONSTANT, value=[255, 255, 255])

    fusion_pred = add_tag(fusion_pred, 'prediction')

    fusion_prev = add_tag(fusion_prev, 'previous prediction')
    fusion_prev = cv2.copyMakeBorder(fusion_prev, 0, 0, 10, 10, cv2.BORDER_CONSTANT, value=[255, 255, 255])

    h, w = image.shape[0], image.shape[1]
    if h < w:
        out_image = cv2.hconcat(
            [image.astype(np.float32), fusion_gt.astype(np.float32), fusion_pred.astype(np.float32),
