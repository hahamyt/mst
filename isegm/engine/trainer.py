...
from isegm.utils.log import logger, TqdmToLogger, SummaryWriterAvg, tensor2pillow, plt_click_patch, plt_grid
...
class ISTrainer(object):
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        self.rank = rank
        self.writer = SummaryWriter(log_dir=f'runs/{cfg.exp_name}')
        ...
        self.train_data = DataLoader(
            ...
            num_workers=cfg.workers, pin_memory_device=f'cuda:{rank}'
        )
    ...
    def run(self, num_epochs, start_epoch=None, validation=True):
        ...
        if self.cfg.amp:
            self.scaler = torch.cuda.amp.GradScaler(growth_factor=2.0, backoff_factor=0.5, enabled=self.cfg.amp, growth_interval=20)
    ...
    def training(self, epoch):
        ...
        tbar = tqdm(self.train_data) if self.is_master else self.train_data # #, file=self.tqdm_out, ncols=100)\
        ...
        for i, batch_data in enumerate(tbar):
            ...
            if self.is_master:
                ...
                self.sw.add_scalar(tag=f'{log_prefix}States/learning_rate',
                                   ...
                                   global_step=global_step)
                # self.wandb.log({'loss': train_loss / (i + 1), "ema_iou": self.train_metrics[0]._ema_iou})
                self.writer.add_scalar('EMA IOU:', self.train_metrics[0]._ema_iou, global_step=self.global_step)
        ...
        if self.is_master:
            ...
            if epoch % checkpoint_interval == 0:
                save_checkpoint(self.net, self.cfg.CHECKPOINTS_PATH, prefix=self.task_prefix,
                                epoch=epoch, multi_gpu=self.cfg.distributed)
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            if random.random() < 0.02:
                ...
                points[:] = -1
                points = get_next_points(prev_output, gt_mask, points, 1)
            else:
                # iter mask (RITM)
            ...
            output = self._forward(net_input, points)
            ...
            loss = self.add_loss(
                ...
                losses_logging,
                validation,
                lambda: (output['tokens'], batch_data['instances']))
            loss = self.add_loss(
                'score_loss',
                loss,
                losses_logging,
    ...
    def find_next_n_points(self, image, gt_mask, points, prev_output,
        ...
        with torch.set_grad_enabled(grad):
            ...
            for click_idx in range(num_points):
                ...
                points = get_next_points(prev_output, gt_mask, points, click_idx+1)
    ...
    def _forward(self, net_input, points, *args, **kwargs):
        # handle autocast for automatic mixed precision
        if self.cfg.amp:
            with torch.cuda.amp.autocast():
                output = self.net(net_input, points, *args, **kwargs)
        else:
            output = self.net(net_input, points, *args, **kwargs)
        return output
    ...
    def add_loss(self, loss_name, total_loss, losses_logging, validation, lambda_loss_inputs):
        ...
        if loss_weight > 0.0:
            ...
            self.writer.add_scalar(loss_name + '_loss', loss, global_step=self.global_step)
